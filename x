...
-- Simplified Shooting Logic (without advanced prediction)
            ...
            if isMouseOnTarget(TargetPlayer) then
                ...
                for _, part in pairs(bodyParts) do
                    ...
                    if onScreen and isWithinViewEra(part.Position) then
                        -- Use target's position without advanced prediction (simple approach)
                        syncBoxWithTarget(screenPos)
                        
                        -- Ensure cooldown before clicking
                        if os.clock() - lastClickTime >= shared.Forbidden.TriggerBot.Settings.Cooldown then
                            lastClickTime = os.clock()

                            -- Perform click action with the correct tool
